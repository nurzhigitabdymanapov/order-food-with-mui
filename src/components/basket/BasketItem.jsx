...
export const BasketItem = ({ title, price, amount, id }) => {
  ...
  const incrementFoodHandler = async () => {
    try {
      ...
      dispatch(snackbarActions.doSuccesIncrement());
    } catch (error) {
      dispatch(snackbarActions.doErrorIncrement());
    }
  };

  const decrementFoodHandler = () => {
    try {
      dispatch(decrementFood({ id: id, amount: amount - 1 })).unwrap();
      dispatch(snackbarActions.doSuccesDecrement());
    } catch (error) {
      dispatch(snackbarActions.doErrorDecrement());
    }
  };
